...
- `src/components/ResultsQr.tsx` — renders the signed results payload via `qrcode`, with a download button (`canvas.toDataURL('image/png')` → anchor → click), mirroring `apps/web`'s `challenge-qr.tsx` pattern, for the PC to scan back via webcam.
